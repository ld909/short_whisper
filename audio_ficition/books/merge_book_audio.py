...
# 自定义倍速：1.25倍速合并中文书籍音频
python merge_book_audio.py --speed 1.25 --lang zh

# 自定义倍速：1.2倍速合并指定UUID的书籍
...
def get_book_directories(base_dir, dir_type="text"):
    ...
    if skipped_dirs:
        print(
            f"🚫 在{dir_type}目录跳过 {len(skipped_dirs)} 个Mac系统目录: {', '.join(skipped_dirs)}"
        )
...
def get_chunk_files(directory, uuid, file_type="txt"):
    ...
    if skipped_files:
        print(
            f"🚫 [UUID:{uuid[:8]}...] 跳过 {len(skipped_files)} 个Mac系统{file_type}文件: {', '.join(skipped_files)}"
        )
...
def check_file_consistency(uuid, text_dir, audio_dir):
    ...
    print(
        f"📊 [UUID:{uuid[:8]}...] 文本文件数量: {len(text_files)} (索引: {min(text_indices) if text_indices else 'N/A'}-{max(text_indices) if text_indices else 'N/A'})"
    )
    print(
        f"📊 [UUID:{uuid[:8]}...] 音频文件数量: {len(audio_files)} (索引: {min(audio_indices) if audio_indices else 'N/A'}-{max(audio_indices) if audio_indices else 'N/A'})"
    )
    ...
    for index in audio_indices:
        ...
        if audio_file and not is_valid_audio_file(audio_file):
            ...
            print(
                f"❌ [UUID:{uuid[:8]}...] 无效音频文件: {os.path.basename(audio_file)}"
            )

    # 判断一致性
    is_consistent = (
        len(text_files) == len(audio_files)
        and text_indices_set == audio_indices_set
        and not invalid_audio
    )
...
def merge_audio_files(uuid, audio_files, output_file, temp_dir, audio_speed=1.0):
    ...
    for audio_file in audio_files:
        if is_valid_audio_file(audio_file):
            ...
            print(
                f"⚠️  [UUID:{uuid[:8]}...] 跳过无效音频片段: {os.path.basename(audio_file)}"
            )
    ...
    if len(valid_audio_files) != len(audio_files):
        print(
            f"⚠️  [UUID:{uuid[:8]}...] 有 {len(audio_files) - len(valid_audio_files)} 个无效片段被跳过"
        )
        print(
            f"📝 [UUID:{uuid[:8]}...] 实际将合并 {len(valid_audio_files)} 个有效音频片段"
        )
    ...
    try:
        ...
        print(
            f"🔗 [UUID:{uuid[:8]}...] 准备合并 {len(valid_audio_files)} 个有效音频片段"
        )
        ...
        if result.returncode == 0:
            ...
            if is_valid_audio_file(output_file, MIN_MERGED_AUDIO_SIZE):
                ...
                print(
                    f"📊 [UUID:{uuid[:8]}...] 输出文件大小: {file_size:,} 字节 ({file_size/1024/1024:.1f} MB)"
                )
...
def process_book_by_uuid(
    ...
    if not force_regenerate and is_valid_audio_file(output_file, MIN_MERGED_AUDIO_SIZE):
        file_size = os.path.getsize(output_file)
        print(
            f"⏭️  [UUID:{uuid[:8]}...] 完整音频文件已存在且有效，跳过: {output_file} ({file_size:,} 字节)"
        )
        return {
            "uuid": uuid,
            "status": "skipped",
    ...
    # 检查文件一致性
    is_consistent, text_files, audio_files, missing_audio, invalid_audio = (
        check_file_consistency(uuid, text_dir, audio_dir)
    )
    ...
    if not is_consistent:
        ...
        print(
            f"💡 [UUID:{uuid[:8]}...] 请先运行 synthesize_book_audio.py 确保所有音频片段正确生成"
        )
...
使用示例:
  ...
  python merge_book_audio.py --speed 1.15 --lang zh        # 1.15倍速合并中文书籍音频
    ...
    parser.add_argument(
        "--text-base-dir", help="文本文件基础目录路径 (默认：根据语言自动确定)"
    )
    parser.add_argument(
        "--lang",
        "-l",
    ...
    parser.add_argument(
        "--audio-base-dir", help="音频文件基础目录路径 (默认：根据语言自动确定)"
    )
    parser.add_argument(
        "--output-base-dir", help="输出基础目录路径 (默认：根据语言自动确定)"
    )
    parser.add_argument(
        "--temp-dir",
    ...
    parser.add_argument(
        ...
        "-s",
        type=float,
        default=None,  # 改为None，稍后根据语言设置默认值
        help="音频播放速度倍数 (默认: 中文1.15x, 英文1.0x, 范围: 0.25-4.0)",
    )
    ...
    if args.speed is None:
        if args.lang == "zh":
            args.speed = 1.15  # 中文默认1.15倍速
        else:
            args.speed = 1.0  # 英文默认1.0倍速
        ...
        if all_results:
            ...
            if args.preview:
                ...
                for result in all_results:
                    ...
                    if status == "preview":
                        print(
                            f"  - UUID:{uuid[:8]}...{uuid[-8:]}: 需处理 (音频:{result['audio_count']}个)"
                        )
                    elif status == "skipped":
                ...
                for result in all_results:
                    ...
                    if status == "success":
                        print(
                            f"  ✅ UUID:{uuid[:8]}...{uuid[-8:]}: 合并成功 (音频:{result['audio_count']}个)"
                        )
                    elif status == "failed":
                        reason = result.get("reason", "未知原因")
                        print(
                            f"  ❌ UUID:{uuid[:8]}...{uuid[-8:]}: 合并失败 ({reason})"
                        )
                    elif status == "skipped":
