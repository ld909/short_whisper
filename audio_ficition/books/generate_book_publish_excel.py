...
📥 输入信息:
- 音频文件目录: /mnt/dhl/audio/books/en/mp3/*.mp3 (merge_book_audio.py的输出)

📤 输出信息:
- Excel文件: /mnt/dhl/audio/books/excel/book-en.xlsx
- 列结构: UUID | 是否发布 | 发布时间
...
# 预览模式
python generate_book_publish_excel.py --preview

# 强制重新生成
python generate_book_publish_excel.py --force-regenerate
"""
...
# ============ 配置参数 ============
# 音频文件目录：merge_book_audio.py的输出目录
DEFAULT_AUDIO_DIR = "/mnt/dhl/audio/books/en/mp3"

# Excel输出目录（统一存放所有语种的Excel文件）
DEFAULT_EXCEL_DIR = "excel"

# Excel文件名（支持多语种）
EXCEL_FILENAME = "book-en.xlsx"
...
def is_valid_audio_file(file_path, min_size_bytes=MIN_AUDIO_SIZE):
    ...
    try:
        ...
        if file_size < min_size_bytes:
            print(f"⚠️  音频文件过小，可能损坏: {os.path.basename(file_path)} ({file_size} 字节)")
            return False
        return True
...
def get_audio_files(audio_dir):
    ...
    if skipped_files:
        print(f"🚫 跳过 {len(skipped_files)} 个Mac系统文件: {', '.join(skipped_files[:5])}" + 
              (f" 等..." if len(skipped_files) > 5 else ""))
    
    if invalid_files:
        print(f"⚠️  跳过 {len(invalid_files)} 个无效音频文件: {', '.join(invalid_files[:3])}" + 
...
def create_book_publish_excel(
    ...
    if invalid_files:
        print(f"⚠️  跳过 {len(invalid_files)} 个无效UUID文件: {', '.join(invalid_files[:3])}" + 
              (f" 等..." if len(invalid_files) > 3 else ""))
    ...
    # 找出新的UUID
    existing_uuids = set(existing_df['UUID'].tolist()) if not existing_df.empty else set()
...
使用示例:
  ...
  python generate_book_publish_excel.py --force-regenerate # 强制重新生成
    ...
    parser.add_argument(
        ...
        help=f"音频文件目录路径 (默认: {DEFAULT_AUDIO_DIR})"
    )
    parser.add_argument(
        "--excel-dir", 
    ...
    parser.add_argument(
        ...
        help=f"Excel文件名 (默认: {EXCEL_FILENAME})"
    )
    parser.add_argument(
        "--language", "-l",
        default="en",
    ...
    if args.excel_filename == EXCEL_FILENAME and args.language != "en":
