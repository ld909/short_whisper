...
# 处理指定UUID
python book_processing_controller.py --uuid 12345678-abcd-efgh-ijkl-123456789012
...
class BookProcessingController:
    ...
    def __init__(self, debug=False, force=False, target_uuid=None):
        self.debug = debug
        self.force = force
    ...
    def _get_clips_args(self) -> List[str]:
        ...
        args.extend([
            ...
            "--font-size", "126",
            "--video-center",
            "--summary-font-size", "40",
            "--summary-top-margin", "620",
            "--summary-right-margin", "620",
            "--icon-size", "32",
    ...
    def check_system_compatibility(self, step_number: int) -> bool:
        ...
        if step["ubuntu_only"] and self.system != "Linux":
            print(f"⚠️  步骤 {step_number} ({step['name']}) 仅支持Ubuntu系统，当前系统: {self.system}")
            return False
        return True
    ...
    def run_pipeline(self, steps: Optional[List[int]] = None) -> bool:
        ...
        print(f"📈 成功率: {success_count/(success_count+failed_count)*100:.1f}%" if (success_count+failed_count) > 0 else "N/A")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
使用示例:
  python book_processing_controller.py --full-pipeline               # 运行完整流水线
  python book_processing_controller.py --steps 1,2,3                 # 运行指定步骤
  python book_processing_controller.py --steps 1-5                   # 运行步骤范围
  python book_processing_controller.py --info                        # 显示流水线信息
  python book_processing_controller.py --uuid xxx --full-pipeline    # 处理指定UUID
...
- 使用 --debug 启用调试模式查看详细信息
    ...
    if args.full_pipeline:
        ...
        print("🔥 运行完整流水线（步骤 1-9）")
    else:
        steps = parse_step_list(args.steps)
        if not steps:
            print("❌ 没有指定有效的步骤")
            sys.exit(1)
        print(f"🎯 运行指定步骤: {steps}")
