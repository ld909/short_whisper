...
def save_progress_log(output_dir, stats, successful_count=0, failed_count=0):
    ...
    try:
        ...
        progress_data = {
            ...
            "remaining_files": stats["remaining_files"]
            - successful_count
            - failed_count,
            "completion_rate": (
                (
                    (stats["completed_files"] + successful_count)
                    / stats["total_files"]
                    * 100
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="音频合成器 - 使用 f5-tts 将文本块合成为音频（支持断点续传）"
    )
    parser.add_argument(
        "--input-dir", default=INPUT_DIR, help=f"输入目录路径 (默认: {INPUT_DIR})"
    )
    parser.add_argument(
        "--output-dir", default=OUTPUT_DIR, help=f"输出目录路径 (默认: {OUTPUT_DIR})"
    )
    parser.add_argument(
        "--ref-audio", default=REF_AUDIO, help=f"参考音频文件路径 (默认: {REF_AUDIO})"
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument("--story", help="只处理指定故事索引的文本块（可选）")
    parser.add_argument(
        "--chunk", help="只处理指定块索引的文本块（需要同时指定 --story）"
    )
    parser.add_argument(
        "--preview",
        action="store_true",
    ...
    if args.story:
        chunk_files = [
            (story_idx, chunk_file)
            for story_idx, chunk_file in chunk_files
    ...
    try:
        ...
        for i, (story_index, chunk_file) in enumerate(stats["pending_files"], 1):
            ...
            print(
                f"总体进度: {stats['completed_files'] + i}/{stats['total_files']} ({(stats['completed_files'] + i)/stats['total_files']*100:.1f}%)"
            )
        ...
        print(
            f"📊 总体完成: {stats['completed_files'] + successful_count}/{stats['total_files']} ({(stats['completed_files'] + successful_count)/stats['total_files']*100:.1f}%)"
        )
