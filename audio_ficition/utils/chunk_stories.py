...
📤 输出信息:
- 默认输出根目录: /home/dhl/Documents/audio/chunks/
- 各主题输出目录: /home/dhl/Documents/audio/chunks/{theme}/
- 输出目录结构: chunks/{theme}/{story_index}/{chunk_index}.txt
- 例如: chunks/scifi/1/1.txt, chunks/scifi/1/2.txt...
- 每个故事有自己的子目录，按故事索引命名
- 每个子目录内的文件按块索引命名
...
def process_theme(theme, input_base_dir, output_base_dir, max_chars=3000, preview_mode=False, resume_mode=True):
    """
    处理指定主题的所有故事文件
    ...
    if skipped_count > 0:
        print(f"📋 跳过 {skipped_count} 个已处理故事，还需处理 {len(files_to_process)} 个故事")
    ...
    for i, input_file in enumerate(files_to_process, 1):
        ...
        success, chunk_count = process_single_story(input_file, output_dir, max_chars, theme_name)
...
使用示例:
  ...
  python chunk_stories.py --no-resume              # 禁用断点续传
    ...
    if args.file:
        ...
        success, chunk_count = process_single_story(input_file, single_file_output_dir, args.max_chars)
    ...
    for theme in theme_list:
        try:
            result = process_theme(theme, input_base_dir, output_base_dir, max_chars, args.preview, resume_mode)
    ...
    if all_results:
        ...
        if args.preview:
            ...
            print(f"\n📋 各主题详情:")
            for result in all_results:
                print(f"  - {result['theme_name']}: 成功 {result['successful']} 个，失败 {result['failed']} 个，跳过 {result['skipped']} 个，生成 {result['total_chunks']} 块")
            
            if total_successful > 0:
                print(f"\n📝 分块后的文件已保存到: {output_base_dir}")
                print(f"📁 目录结构: chunks/{theme}/{story_index}/{chunk_index}.txt")
