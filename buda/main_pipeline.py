...
class PipelineController:
    ...
    def run_script(self, script_name, step_number, description):
        ...
        try:
            ...
            result = subprocess.run(
                ...
                cwd=self.script_dir,
                # 移除 capture_output=True，让子进程输出直接显示到终端
                text=True,
                timeout=3600,  # 1小时超时
            )
    ...
    def run_pipeline(self):
        ...
        steps = [
            ...
            (
                ...
                "4.1",
                "检查 mp3 进度，删除坏 mp3，重新生成对应 mp3 clip",
            ),
        ...
        for script_name, step_num, description in steps:
            ...
            if not success:
                ...
                # 询问是否继续
                user_input = input(
                    f"\n步骤 {step_num} 失败。是否继续执行下一步？(y/n/q): "
                ).lower()
                if user_input == "q":
                    print("❌ 用户选择退出管道执行")
                    break
    ...
    def generate_report(self, start_time, failed_steps):
        ...
        print(
            f"成功率: {(self.success_count / (len(failed_steps) + self.success_count)) * 100:.1f}%"
        )
