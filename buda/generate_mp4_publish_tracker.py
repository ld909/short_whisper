...
- 扫描输出目录中的所有MP4文件
- 为每种语言创建一个Excel工作表
- 跟踪四个字段：MP4名称、是否发布（默认1）、是否已经发布（默认0）、发布时间（默认空）
...
def scan_mp4_files(base_dir, specific_channel=None):
    ...
    # 获取所有频道目录
    if specific_channel:
        channel_dirs = [specific_channel] if os.path.isdir(os.path.join(base_dir, specific_channel)) else []
        if not channel_dirs:
            print(f"错误: 指定的频道目录不存在 {os.path.join(base_dir, specific_channel)}")
            return {}
    else:
        channel_dirs = [
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                if len(df.columns) >= 3:
                    if len(df.columns) == 3:
                        ...
                        # 新版本，有4列或更多
                        df.columns = ['MP4名称', '是否发布', '是否已经发布', '发布时间'] + list(df.columns[4:])
...
def update_tracker(language_files, existing_data, output_file):
    ...
    for language, files in language_files.items():
        ...
        if language in existing_data:
            ...
            df = pd.DataFrame(columns=['MP4名称', '是否发布', '是否已经发布', '发布时间'])
        ...
        # 按文件名排序
        df = df.sort_values('MP4名称').reset_index(drop=True)
...
def parse_args():
    ...
    parser.add_argument("-c", "--channel", help="指定要处理的频道名（可选，默认处理所有频道）")
    ...
    parser.add_argument("--list-languages", action="store_true", help="列出所有可用语言")
