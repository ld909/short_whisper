...
def check_supported_system():
    ...
    if system == "Darwin":
        ...
        # Linux 系统，进一步检查是否为 Ubuntu
        try:
            with open("/etc/os-release", "r") as f:
                content = f.read()
                if "Ubuntu" in content:
                    print(f"✅ 系统检查通过：Ubuntu Linux")
                    return True
                else:
                    print(f"✅ 系统检查通过：Linux (假设兼容)")
                    return True
...
def add_music_to_video(
    ...
    try:
        ...
        if audio_duration < video_duration:
            ...
            # 使用复杂滤镜实现音频循环和时长裁剪
            audio_filter = (
                f"[1:a]aloop=loop={loop_count-1}:size={int(audio_duration * 48000)}[looped];"
        ...
        if os.path.exists(output_file):
            file_size = os.path.getsize(output_file)
            print(
                f"📊 输出文件大小: {file_size:,} bytes ({file_size/1024/1024:.1f} MB)"
            )
            ...
            if abs(output_duration - video_duration) < 0.1:  # 允许0.1秒误差
                ...
                print(
                    f"⚠️ 输出视频时长不匹配: 期望 {video_duration:.2f} 秒，实际 {output_duration:.2f} 秒"
                )
        ...
        # 如果GPU加速失败，尝试回退到软件编码
        if (has_nvenc or has_vaapi or has_qsv) and "not supported" in str(
            e.stderr
        ).lower():
...
支持系统:
   ...
   - Ubuntu: /mnt/dhl/beauty/
    ...
    parser.add_argument(
        "--index", type=int, help="指定要处理的index。不指定时处理所有可用的index"
    )

    parser.add_argument(
        "--force", action="store_true", help="强制重新处理（忽略断点续传）"
    )
