...
def create_youtube_cover(input_path: str, output_path: str, target_width: int = 1280, 
                        target_height: int = 720, quality: int = 90, debug: bool = False) -> bool:
    """
    将输入图片转换为YouTube封面尺寸
        ...
        with Image.open(input_path) as img:
            ...
            if abs(original_ratio - target_ratio) < 0.01:
                ...
                if original_ratio / target_ratio > 1.5:
                    ...
                    img_cropped = img.crop((crop_x, 0, crop_x + new_width, original_height))
                    final_img = img_cropped.resize((target_width, target_height), Image.Resampling.LANCZOS)
                ...
                if target_ratio / original_ratio > 1.5:
                    ...
                    img_cropped = img.crop((0, crop_y, original_width, crop_y + new_height))
                    final_img = img_cropped.resize((target_width, target_height), Image.Resampling.LANCZOS)
...
def main():
    ...
    for i, filename in enumerate(status["remaining"], 1):
        ...
        if create_youtube_cover(input_path, output_path, quality=args.quality, debug=args.debug):
            success_count += 1
            print(f"   ✅ 成功")
        else:
            error_count += 1
