...
输入路径: /mnt/dhl/beauty/clips/[index]/*.mp4
输出路径: /mnt/dhl/beauty/mp4_low_res/[index].mp4
"""
...
合并规则:
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help="试运行模式，只显示合并计划，不实际执行"
    )
    
    args = parser.parse_args()
    
    # 构建路径
    clips_dir = f"/mnt/dhl/beauty/clips/{args.index}"
    output_path = f"/mnt/dhl/beauty/mp4_low_res/{args.index}.mp4"
